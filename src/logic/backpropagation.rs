...
fn calculate_error_rate_per_layer(
    ...
    for layer in layers.iter().skip(1).rev() {
        ...
        result.push(current_matrix.multiply(result_from_last_processed_layer)?);
    }
    result.reverse();
...
fn new_weights_based_on_error_rate_and_gradient_descent(
    ...
    for (i, layer) in layers.iter().enumerate() {
        let mut weight_adjustments = error_rate_per_layer.0[i]
            .multiply(&feedforward.results()[i + 1])?
            .multiply(
                &feedforward.results()[i + 1]
                    .clone()
                    .one_minus_all_values(),
            )?
            .multiply(&feedforward.results()[i].transpose())?;
        weight_adjustments.multiply_value_to_all_values_in_matrix(learning_rate);
        let mut new_layer = layer.clone().get_matrix();
        new_layer.add_matrix_of_same_size(&weight_adjustments).unwrap();
        new_layers.push(Layer::new(new_layer));
    }
    Ok(new_layers)
}
...
mod tests {
    ...
    fn calculate_error_rate_example_from_the_book() {
        ...
        .0;
        let expected = vec![
            Matrix::new(vec![vec![0.7], vec![1.3]]),
            Matrix::new(vec![vec![1.5], vec![0.5]]),
        ];
